...
class Workspace(object):
        ...
        for r in range(repeat):            
            ...
            for episode in range(self.cfg.num_eval_episodes):
                ...
                while not done:                
                    ...
                    #self.eval_env.render(width=500, height =500) #For rendering during evaluation
    ...
    def _run(self):        
        ...
        while self.step <= self.cfg.num_train_steps:
            ...
            if done:
                ...
                if self.cfg.use_hgg:                    
                    ...
                    while True:
                        ...
                        sampled_goal = hgg_sampler.sample(np.random.randint(len(hgg_sampler.pool))).copy() #Curriculum Goal generated by OUTPACE
                    ...
                    if self.step != 0 and self.cfg.use_inpainting:
                        obs = self.env.reset(goal = inpaint_goal) # This is state_0, we set our methods curriculum goal here
                        generated_curriculum_points.append(obs)
                    else: 
                        obs = self.env.reset()
                ...
                if self.cfg.render_images:
                    ...
                    ''' #Moved them to cfg file keeping this here just in case
                    if self.cfg.env == "PointSpiralMaze-v0":
                        image_scale = 25
                        agent_scale = round(image_scale*1.5) #updated for spiral maze to be changed
                    elif self.cfg.env == "sawyer_peg_push":
                        image_scale = 250
                        agent_scale = image_scale//15
                    elif self.cfg.env == "sawyer_peg_pick_and_place":
                        image_scale = 250
                        agent_scale = image_scale//15
                        z_ratio = 4 #Since the max dimensions of x and z are different by almost 3 times and z should be more precise, it is scaled by 4
                    else:
                        image_scale = 25
                        agent_scale = image_scale
                    '''

                    image_scale = self.cfg.image_scale
                    agent_scale = self.cfg.agent_scale

                    goal_scale = agent_scale//2
                    ...
                    if self.cfg.env == "sawyer_peg_pick_and_place":
                        z_ratio = self.cfg.z_scale #Just in case
                    ...
                    for i in range(len(positions)):
                        if self.cfg.env == "sawyer_peg_pick_and_place":
                            agent_scale = image_scale//15 #This is here to keep agent scale consistent with xy and yz scale, since we update yz scale below
                        cur_val_y = int(round((positions[i][0] - self.uniform_goal_sampler.LOWER_CONTEXT_BOUNDS[0])*image_scale)) #Flip x and y
                        cur_val_x = int((maze_dim_x*image_scale)-1 - round((positions[i][1] - self.uniform_goal_sampler.LOWER_CONTEXT_BOUNDS[1])*image_scale)) #Flip x and y
                        ...
                        #Sanity checks
                        if cur_val_x+(agent_scale//2 + 1) >= maze_dim_x*image_scale:
                            cur_val_x = int((maze_dim_x*image_scale) - ((agent_scale//2) +2))
                        if cur_val_y+(agent_scale//2 + 1) >= maze_dim_y*image_scale:
                            cur_val_y = int((maze_dim_y*image_scale) - ((agent_scale//2) +2))
                        if cur_val_x-(agent_scale//2 + 1) <= 0:
                            cur_val_x = int((agent_scale//2)+2)
                        if cur_val_y-(agent_scale//2 + 1) <= 0:
                            cur_val_y = int((agent_scale//2)+2)
                        ...
                        if (self.cfg.env == "sawyer_peg_push" or self.cfg.env == "sawyer_peg_pick_and_place") and max_movement == 0 and i == 0: #to show the agent if the object does not move
                            for k in range(agent_scale):
                                for n in range(agent_scale):
                                    if ((k < agent_scale//3 and n < agent_scale//3) or (k > (agent_scale-agent_scale//3) and n > (agent_scale-agent_scale//3)) 
                                        or (k < agent_scale//3 and n > (agent_scale-agent_scale//3)) or (k > (agent_scale-agent_scale//3) and n < agent_scale//3)):
                                        continue
                                    cur_image_bw[prev_val_x-(agent_scale//2)+k][prev_val_y-(agent_scale//2)+n] += 1
                        ...
                        for j in range(max_movement): #move coordinate towards next coordinate and mark the ones along the way
                            if abs(current_coordinate_y - cur_val_y) > 0 and abs(current_coordinate_x - cur_val_x) > 0: #move diagonally
                                if current_coordinate_y > cur_val_y and current_coordinate_x > cur_val_x:
                                    current_coordinate_y -= 1
                                    current_coordinate_x -= 1
                                elif current_coordinate_y < cur_val_y and current_coordinate_x < cur_val_x:
                                    current_coordinate_y += 1
                                    current_coordinate_x += 1
                                elif current_coordinate_y > cur_val_y and current_coordinate_x < cur_val_x:
                                    current_coordinate_y -= 1
                            ...
                            for k in range(agent_scale): #Plus shaped agent's trajectory
                                for n in range(agent_scale):
                                    if ((k < agent_scale//3 and n < agent_scale//3) or (k > (agent_scale-agent_scale//3) and n > (agent_scale-agent_scale//3)) 
                                        or (k < agent_scale//3 and n > (agent_scale-agent_scale//3)) or (k > (agent_scale-agent_scale//3) and n < agent_scale//3)):
                                        continue
                                    cur_image_bw[current_coordinate_x-(agent_scale//2)+k][current_coordinate_y-(agent_scale//2)+n] += 1
                        prev_val_y = cur_val_y
                        prev_val_x = cur_val_x
                    ...
                    cur_image_medium_goal = copy.deepcopy(cur_image)
                    cur_image_medium_goal_black = copy.deepcopy(cur_image)
                    ...
                    #Sanity checks
                    if cur_goal_x +(goal_scale//2 + 1) >= maze_dim_x*image_scale:
                        cur_goal_x = int((maze_dim_x*image_scale) - ((goal_scale//2) +2))
                    if cur_goal_y +goal_scale//2 >=maze_dim_y*image_scale:
                        cur_goal_y = int((maze_dim_y*image_scale) - ((goal_scale//2) +2))
                    if cur_goal_x-(goal_scale//2 + 1) <= 0:
                        cur_goal_x = int((goal_scale//2)+2)
                    if cur_goal_y-(goal_scale//2 + 1) <= 0:
                        cur_goal_y = int((goal_scale//2)+2)

                    for k in range(goal_scale): #Save goal on image
                        for n in range(goal_scale):
                            cur_image[cur_goal_x-(goal_scale//2)+k][cur_goal_y-(goal_scale//2)+n] = [1,0.8,0.2]
                            cur_image_black[cur_goal_x-(goal_scale//2)+k][cur_goal_y-(goal_scale//2)+n] = [0,0,0] #Small black goal
                    ...
                    for k in range(goal_scale):
                        for n in range(goal_scale):
                            cur_image_medium_goal[cur_goal_x-(goal_scale//2)+k][cur_goal_y-(goal_scale//2)+n] = [1,0.8,0.2]
                            cur_image_medium_goal_black[cur_goal_x-(goal_scale//2)+k][cur_goal_y-(goal_scale//2)+n] = [0,0,0]

                    if not os.path.exists("heatmaps_medium"):
                        os.makedirs("heatmaps_medium")
                    cv2.imwrite(f"heatmaps_medium/heatmap_episode{episode}.png", (cur_image_medium_goal*255).astype('uint8'))

                    if not os.path.exists("heatmaps_medium_black"):
                        os.makedirs("heatmaps_medium_black")
                    cv2.imwrite(f"heatmaps_medium_black/heatmap_episode{episode}.png", (cur_image_medium_goal_black*255).astype('uint8'))
                    ...
                    for k in range(goal_scale):
                        for n in range(goal_scale):
                            cur_image_large_goal_black[cur_goal_x-(goal_scale//2)+k][cur_goal_y-(goal_scale//2)+n] = [0,0,0]
                    ...
                    if self.cfg.env in ['sawyer_peg_pick_and_place']: #Since this env has xy and yz yz is saved here, same process though
                        
                        agent_scale = image_scale//10 #To keep the agent size consistent with xy image
                        ...
                        for i in range(len(positions)):
                            ...
                            if cur_val_y+(agent_scale//2 + 1) >= maze_dim_y*image_scale:
                                cur_val_y = int((maze_dim_y*image_scale) - ((agent_scale//2) +2))
                            if cur_val_z+(agent_scale//2 + 1) >= maze_dim_z*z_scale:
                                cur_val_z = int((maze_dim_z*z_scale) - ((agent_scale//2) +2))
                            if cur_val_y-(agent_scale//2 + 1) <= 0:
                                cur_val_y = int((agent_scale//2)+2)
                            if cur_val_z-(agent_scale//2 + 1) <= 0:
                                cur_val_z = int((agent_scale//2)+2)
                            ...
                            if max_movement == 0 and i == 0: #to show if the object does not move
                                for k in range(agent_scale):
                                    for n in range(agent_scale):
                                        if ((k < agent_scale//3 and n < agent_scale//3) or (k > (agent_scale-agent_scale//3) and n > (agent_scale-agent_scale//3)) 
                                            or (k < agent_scale//3 and n > (agent_scale-agent_scale//3)) or (k > (agent_scale-agent_scale//3) and n < agent_scale//3)):
                                            continue
                                        cur_image_bw_yz[prev_val_y-(agent_scale//2)+k][prev_val_z-(agent_scale//2)+n] += 1
                            ...
                            for j in range(max_movement): #move coordinate towards next coordinate and mark the ones along the way
                                ...
                                if abs(current_coordinate_z - cur_val_z) > 0 and abs(current_coordinate_y - cur_val_y) > 0: #move diagonally
                                    if current_coordinate_z > cur_val_z and current_coordinate_y > cur_val_y:
                                        current_coordinate_z -= 1
                                        current_coordinate_y -= 1
                                    elif current_coordinate_z < cur_val_z and current_coordinate_y < cur_val_y:
                                        current_coordinate_z += 1
                                        current_coordinate_y += 1
                                    elif current_coordinate_z > cur_val_z and current_coordinate_y < cur_val_y:
                                        current_coordinate_z -= 1
                                ...
                                for k in range(agent_scale):
                                    for n in range(agent_scale):
                                        if ((k < agent_scale//3 and n < agent_scale//3) or (k > (agent_scale-agent_scale//3) and n > (agent_scale-agent_scale//3)) 
                                            or (k < agent_scale//3 and n > (agent_scale-agent_scale//3)) or (k > (agent_scale-agent_scale//3) and n < agent_scale//3)):
                                            continue
                                        cur_image_bw_yz[current_coordinate_y-(agent_scale//2)+k][current_coordinate_z-(agent_scale//2)+n] += 1
                            prev_val_z = cur_val_z
                            prev_val_y = cur_val_y
                        ...
                        cur_image_yz_medium_goal = copy.deepcopy(cur_image_yz)
                        cur_image_yz_medium_goal_black = copy.deepcopy(cur_image_yz)
                        ...
                        #Sanity checks
                        if cur_goal_y +(goal_scale//2 + 1) >= maze_dim_y*image_scale:
                            cur_goal_y = int((maze_dim_y*image_scale) - ((goal_scale//2) +2))
                        if cur_goal_z +(goal_scale//2 + 1) >=maze_dim_z*z_scale:
                            cur_goal_z = int((maze_dim_z*z_scale) - ((goal_scale//2) +2))
                        if cur_goal_y-(goal_scale//2 + 1) <= 0:
                            cur_goal_y = int((goal_scale//2)+2)
                        if cur_goal_z-(goal_scale//2 + 1) <= 0:
                            cur_goal_z = int((goal_scale//2)+2)

                        for k in range(goal_scale):
                            for n in range(goal_scale):
                                cur_image_yz[cur_goal_y-(goal_scale//2)+k][cur_goal_z-(goal_scale//2)+n] = [1,0.8,0.2]
                                cur_image_yz_black[cur_goal_y-(goal_scale//2)+k][cur_goal_z-(goal_scale//2)+n] = [0,0,0]
                        ...
                        for k in range(goal_scale):
                            for n in range(goal_scale):
                                cur_image_yz_medium_goal[cur_goal_y-(goal_scale//2)+k][cur_goal_z-(goal_scale//2)+n] = [1,0.8,0.2]
                                cur_image_yz_medium_goal_black[cur_goal_y-(goal_scale//2)+k][cur_goal_z-(goal_scale//2)+n] = [0,0,0]

                        if not os.path.exists("heatmaps_yz_medium"):
                            os.makedirs("heatmaps_yz_medium")
                        cv2.imwrite(f"heatmaps_yz_medium/heatmap_episode{episode}.png", (cur_image_yz_medium_goal*255).astype('uint8'))

                        if not os.path.exists("heatmaps_yz_medium_black"):
                            os.makedirs("heatmaps_yz_medium_black")
                        cv2.imwrite(f"heatmaps_yz_medium_black/heatmap_episode{episode}.png", (cur_image_yz_medium_goal_black*255).astype('uint8'))
                        ...
                        for k in range(goal_scale):
                            for n in range(goal_scale):
                                cur_image_yz_large_goal_black[cur_goal_y-(goal_scale//2)+k][cur_goal_z-(goal_scale//2)+n] = [0,0,0]
                ...
                if self.cfg.use_inpainting:
                    ...
                    while not goal_coordinates:
                        if self.cfg.env not in ['sawyer_peg_pick_and_place']:
                            output_imgs = single_img((cur_image_no_goal*255).astype('uint8'),True, f"inpaint_results/episode_{episode}/",
                                                        model_pth = cur_model_dir,
                                                        diffusion_n_steps = self.cfg.diffusion_n_steps, 
                            ...
                            output_imgs_xy = single_img((cur_image_no_goal*255).astype('uint8'),True, f"inpaint_results_xy/episode_{episode}/",
                                                        model_pth = cur_model_dir_xy,
                                                        diffusion_n_steps = self.cfg.diffusion_n_steps, 
                                                        diffusion_schedule= self.cfg.diffusion_schedule,
                                                        diffusion_noise_min = self.cfg.diffusion_noise_min,
                                                        diffusion_noise_max = self.cfg.diffusion_noise_max,
                    ...
                    if not self.cfg.use_inpainting_aim and not self.cfg.use_inpainting_q:
                        ...
                        if self.cfg.use_inpainting_q_final: #Does not work well
                            ...
                            if self.cfg.env in ['sawyer_peg_pick_and_place']:
                                ...
                                os.rename(f"inpaint_results/episode_{episode}/{selected_images[optimal_goal_index]}",f"inpaint_results/episode_{episode}/selected.png")
                            inpaint_goal = goal_candidates[optimal_goal_index]
                        elif self.cfg.use_inpainting_q_initial: #Does not work well
                            cur_obs_list = []
                            cur_actions = []
                            if episode == 0: # sample random action for episode 0
                        ...
                        probabilities = softmax(merge_q_means_aim).flatten()
                        optimal_goal = np.random.choice(Q_means, p = probabilities)
                        optimal_goal_index = np.argmax(Q_means==optimal_goal)
                        if self.cfg.env in ['sawyer_peg_pick_and_place']:
                            os.rename(f"inpaint_results_xy/episode_{episode}/{selected_images_xy[optimal_goal_index]}",f"inpaint_results_xy/episode_{episode}/selected.png")
            ...
            #self.env.render() #To render the env during training, not recommended
            ...
            if self.cfg.use_residual_randomwalk:
                if self.env.is_residual_goal:
                    if (self.env.residual_goalstep % 10 == 0) or info.get('is_current_goal_success'):
                        if (self.cfg.use_uncertainty_for_randomwalk not in [None, 'none', 'None']) and self.step > self.get_agent().meta_test_sample_size: #OUTPACE randomwalk
                            ...
                            if self.cfg.use_inpainting: #Our randomwalk
                                if not printed:
