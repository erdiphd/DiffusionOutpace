...
def get_uniform_mask(img_shape, uniform_type, dtype='uint8'):
    height, width = img_shape[:2]

    mask = np.zeros((height, width, 1), dtype=dtype)

    if uniform_type == "horizontal":
        interval = height // 12
        m_count = 0
        no_m_count = np.random.randint(0,(interval*2)+1)
        for i in range(height):
            for j in range(width):
                if no_m_count == interval*2:
                    mask[i][j] = 1
            if no_m_count == interval*2:
                m_count += 1
                if m_count == interval:
